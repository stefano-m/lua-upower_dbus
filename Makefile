...
release: flake
	...
	@rm -f RELEASE
	@git reset HEAD~1 -- VERSION README.md src/$(project)/init.lua
...
ifndef LUA_UPOWER_DBUS_VERSION
	$(error LUA_UPOWER_DBUS_VERSION must be defined)
endif
	@luarocks upload --api-key=$(LUAROCKS_API_KEY) rockspec/$(project)-$(LUA_UPOWER_DBUS_VERSION).rockspec
